...
export const SimulationForm = (
  ...
  return {
    ...
    getFormData: async function () {
      ...
      if(action){
        ...
        .then((res: any) => {
          ...
          return service.get(`/master/getDetailById?masterName=com.act21.hyperform3.entity.master.rule.Rule&id=${res?.data?.payload?.config?.ruleId}`)
        })
       .then((response : any) => {
    ...
    getUiSchema: async function () {
      const action =  store.searchParams?.get("id");
      let uiSchema:any = SimulationFormUiSchema;let formData2:any ={};
      let data: any = null;
    ...
    setRule: async function(uiSchema : any,action: number) {
      ...
      const Api =
        ...
        uiSchema =await service.get(Api)
        .then((res: any) => {
          if(res?.data?.payload?.config?.ruleId == undefined){
            return undefined;
          }
    ...
    setArtifact : async(uiSchema : any,groupId : string) => {
      let data :any = null;
      await service.post("/rule/getArtifacts", {
        "payload": {
          "groupId":groupId
        },
      })
    ...
    setVersion : async(uiSchema : any, groupId : string, project : string) => {
      let data :any = null;
      await service.post("/rule/getVersions", {
        "payload": {
          "groupId":groupId,
          "artifactId": project
        },
    ...
    onChange: async function(value: any){
      ...
      if (ruleIdentifier && store.newData?.groupId && store.newData?.groupId != store.formData?.groupId) {
        this.setArtifact(uiSchema,store?.newData?.groupId)
      } else if (ruleIdentifier && store.newData?.project && store.newData?.project != store.formData?.project) {
      ...
      await service
        ...
        .then((response: any) => {
          const result =
            ...
            const result2 =
            response.data.payload.config.features.simulationConfig.simulationTypes;
      ...
      if (
        ...
        await service
          ...
          .then((response) => {
           ...
           SimulationFormUiSchema.elements[5].elements[0].config.main.allRowsData = response;
    ...
    loadData: async function () {
      ...
      if (
        ...
        store.setValidation("ValidateAndShow");
        store.setNotify({
          FailMessage: "Please select Program or Upload file to Load or save the data",
          Fail: true,
        });
      } else {
    ...
    deleteRecords: (value: any) => {
      try {
       ...
       let loadRecords= store.ctx.core.data.LoadRecords;
        ...
        arr1=arr;
        store.setFormdata({...store.ctx.core.data,LoadRecords:externalDataIds});
        store.setUiSchema(SimulationFormUiSchema);
    ...
    saveData: async () => {
        ...
        await service.post('/rule/getRule', {
          "payload": {
            'groupId': groupId,
            'artifactId': project,
            'version': version
          ...
          return service
          .get(`/program/getById?id=${programId} `)
        ...
        const result = await service
          ...
          .then((response: any) => {
            ...
            store.setFormdata(formData);
            store.setNotify({SuccessMessage:"Data saved successfully",Success:true,})
        });
      ...
      runSimulation: async () =>{
       ...
       const result = await service
          ...
          .then((response: any) => {
            store.setNotify({SuccessMessage:"Simulation run succesfully",Success:true,});
