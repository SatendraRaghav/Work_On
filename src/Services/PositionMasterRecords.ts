...
export const PositionMasterRecords = (
  ...
  return {
    ...
    View_Actions: function () {
      let businessKey = `PositionNewStaging_${dynamicData?.rowData.id}`;
    ...
    Edit_Approve_Records: function () {
      localStorage.setItem("disabled", "false");
      store.navigate(
        `/PositionMaster?id=${dynamicData?.rowData.id}&disabled=false`
      );
    },
    View_Records: function () {
      localStorage.setItem("disabled", "true");
      store.navigate(`/PositionMasterView?id=${dynamicData?.rowData.id}`);
    }
  };
};
